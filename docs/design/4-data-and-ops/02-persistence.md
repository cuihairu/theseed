...
```
BigWorld 的持久化不是单表 CRUD：
  - Archiver 负责归档
  - SecondaryDB 负责本地暂存和后续归并
  - 还有合服 / 同步 / 修复工具链
```
...
```
KBEngine 主要是 EntityTable 级的加载与保存，
整体更偏向基础 CRUD，而不是完整数据运维体系。
```
...
共同缺点：
  - 一旦引入归档、同步、修复，边界必须很清楚
```

### theseed 的取舍

```
theseed 把主持久化和本地归档暂存层拆开，
避免把 SecondaryDB 误写成主库，
也避免把工具链能力塞进在线读写接口。
```
